...
export const MediaCard: React.FC<MediaCardProps> = ({
  ...
  const [hoveredItem, setHoveredItem] = useState<'view' | 'toggle' | 'delete' | null>(null);
  ...
  const togglePublic = useMediaTogglePublic();
  ...
  const handleTogglePublic = (e: React.MouseEvent) => {
    e.stopPropagation();
    togglePublic.mutate({ id: file.id, is_public: !isPublic });
    setMenuOpen(false);
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: isSelected
          ? '0 0 0 2px var(--primary), 0 10px 28px rgba(68,69,37,.1)'
          : isHovered
    ...
    >
      ...
      <div
        style={{
          height: '110px',
          background: isSelected ? `color-mix(in srgb, ${config.bg} 80%, var(--primary) 20%)` : config.bg,
          display: 'flex',
          alignItems: 'center',
      ...
      >
        ...
        {/* Checkbox — canto superior esquerdo (modo seleção) */}
        {selectionMode && (
          <div
          ...
          >
            <div
              style={{
                ...
                background: isSelected ? 'var(--primary)' : 'rgba(255,255,255,0.75)',
        ...
        {/* Badge público/privado — canto superior esquerdo (modo normal) */}
        {!selectionMode && (
          <div
            onClick={(e) => e.stopPropagation()}
            style={{ position: 'absolute', top: '9px', left: '9px' }}
            title={isPublic ? 'Público — visível no site' : 'Privado — não aparece no site'}
          >
            <span
              style={{
                display: 'inline-flex',
                ...
                fontWeight: 700,
                letterSpacing: '.3px',
                background: isPublic ? '#d1fae5' : 'rgba(0,0,0,0.10)',
                color: isPublic ? '#065f46' : 'rgba(0,0,0,0.45)',
                backdropFilter: 'blur(4px)',
                userSelect: 'none',
              }}
            >
        ...
        {/* Menu Dot Button — canto superior direito (apenas modo normal) */}
        {!selectionMode && (
          <div
            ref={menuRef}
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: '13px',
                    fontWeight: 600,
                    color: isPublic ? '#065f46' : 'var(--text)',
                    background: hoveredItem === 'toggle' ? 'var(--muted)' : 'transparent',
                    border: 'none',
                    textAlign: 'left',
                    cursor: togglePublic.isPending ? 'not-allowed' : 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '8px',
                    opacity: togglePublic.isPending ? 0.6 : 1,
                  }}
                >
