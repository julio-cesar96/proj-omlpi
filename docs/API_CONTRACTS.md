...
|---|---|---|
| `banners` | Home banner | Hero (Início). **DIVERGÊNCIA CONFIRMADA (16/07/2026):** schema real é `singleType` só com `title` (string) + `text` (richtext). Não existe `image`, `subtitle`, `link`, `order`. A interface `StrapiBanner` no Next.js tem campos que não existem — **ação necessária em `next/` (branch `feature/migration-next`)**: corrigir interface e verificar se o Hero depende de algum desses campos ausentes. |
...
| `sobres` | Texto institucional | **CONFIRMADO (Fase 1):** N registros, um por aba (Quem somos / Resultados do levantamento / Histórico). **DIVERGÊNCIA CONFIRMADA (16/07/2026):** campo de conteúdo real é `text` (não `content`), e **não existe campo `order`** no schema — `_sort: "order:asc"` pode retornar ordem imprevisível. **Ação necessária em `next/`**: corrigir interface e confirmar visualmente a ordem das abas. |
| `textoindicadors` | Texto da página de indicadores | **CONFIRMADO:** é o texto introdutório da Midiateca (ex-`/indicadores`), como parágrafo no topo da seção. **DIVERGÊNCIA CONFIRMADA (16/07/2026):** campos reais são `titulo`/`texto` (português), não `title`/`content`. **Ação necessária em `next/`**: corrigir interface `StrapiTextoIndicador`. |
| `guias` | Guias/documentos de referência | Midiateca / PNIPI — **INVESTIGADO E RESOLVIDO:** não existe content-type "FAQ" ou "Planos de ação" hoje. Schema confirmado: `title` (obrigatório), `description`, `file` (obrigatório), sem Draft & Publish. **Decisão de modelagem adiada intencionalmente** para a fase de redesign do CMS — "Dúvidas frequentes" e "Planos de ação" permanecem como placeholder estático no front (Fase 2) até lá. |
...
| `artigos` | Artigos da biblioteca (busca, tags, paginação) | **CONFIRMADO (Fase 1):** Midiateca usa esta collection para metadados. Schema confirmado: `title`, `description`, `tags`, `file`, `author`, `image`, `date` (⚠️ string, não datetime), `organization`, `youtube`. **A busca/filtro em si vem do `omlpi-cms-search` (ver §3), não daqui.** |
...
| `politica-de-privacidade` | Conteúdo da política de privacidade | **PENDENTE DE VERIFICAÇÃO (16/07/2026):** uma investigação concluiu que a URL real é `privacy-policy` (baseado no `collectionName` do schema), mas esse raciocínio está tecnicamente incorreto para Strapi v3 — `collectionName` define a tabela do banco, não a rota REST (que vem de `config/routes.json`, ainda não verificado). Testar diretamente: `curl https://omlpi-strapi.rnpiobserva.org.br/privacy-policy` vs. `curl .../politica-de-privacidade`, ver qual devolve dado real. Continua sem seção própria no menu — abre como modal (`<PrivacyPolicyModal />`) a partir de um link no Footer. |
...
**CONFIRMADO (15/07/2026), via inspeção direta de `/etc/nginx/sites-available/observa` e teste real:** não existe uma URL/host separado para este serviço. O Nginx roteia por **path**, no mesmo domínio do Strapi: `https://omlpi-strapi.rnpiobserva.org.br/artigos` é redirecionado internamente para o container `cms_search` (porta 2003), enquanto qualquer outra rota nesse mesmo domínio (`/locales`, `/banners`, etc.) vai para o Strapi de verdade (porta 2001). Confirmado com chamada real: `GET /artigos?_q=teste` retorna `{"hasMore":false,"limit":10,"offset":0,"results":[...]}` com dado real.
