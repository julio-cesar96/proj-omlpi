...
import { AlterarSenhaModal } from '../usuarios/AlterarSenhaModal';
...
export const Topbar: React.FC<TopbarProps> = ({ onImportClick }) => {
  ...
  const { alterarMinhaSenha } = useUsuarioMutations();
  ...
  const handleSalvarSenha = async (novaSenha: string) => {
    await alterarMinhaSenha.mutateAsync({ novaSenha });
    // Aguarda 1.5s para o usuário ver a mensagem de sucesso antes de desconectar
