...
const generateFileName = (name, ext) => {
  let baseName = nameToSlug(name, { separator: '_', lowercase: false });
  const configPublicPath = strapi.config.get(
    'middleware.settings.public.path',
    strapi.config.paths.static
  );
  const uploadDir = path.resolve(strapi.dir, configPublicPath);
  let filePath = path.join(uploadDir, `/uploads/${baseName}${ext}`);
  let count = null;

  function handleFileExistance() {
    try {
      if (fs.existsSync(filePath)) {
        console.log(filePath);
        count++;
        filePath = path.join(uploadDir, `/uploads/${baseName}-${count}${ext}`);
      	handleFileExistance();
      }
    } catch(err) {
...
module.exports = {
  formatFileInfo({ filename, type, size }, fileInfo = {}, metas = {}) {
    ...
    const entity = {
      ...
      hash: generateFileName(basename, ext),
      ext,
      mime: type,
